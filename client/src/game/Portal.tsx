...
const frag = /* glsl */ `
  ...
  void main() {
    ...
    vec3 cyan = vec3(0.21, 0.95, 1.0);
    vec3 violet = vec3(0.64, 0.35, 1.0);
    vec3 magenta = vec3(1.0, 0.31, 0.85);
  ...
  return (
    <group position={[x, 0, z]} rotation={[0, rotY, 0]}>
      ...
      <mesh receiveShadow castShadow position={[0, 0.25, 0]}>
        ...
        <meshStandardMaterial color="#1c2033" roughness={0.6} metalness={0.4} />
      </mesh>
      <mesh receiveShadow castShadow position={[0, 0.7, 0]}>
        <cylinderGeometry args={[6.5, 7, 0.5, 8]} />
        <meshStandardMaterial color="#232849" roughness={0.6} metalness={0.4} />
      </mesh>
      <mesh position={[0, 0.98, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <ringGeometry args={[6.4, 6.7, 64]} />
        <meshStandardMaterial color="#35f2ff" emissive="#35f2ff" emissiveIntensity={2} toneMapped={false} side={THREE.DoubleSide} />
      </mesh>
      {/* pillars */}
      {[-1, 1].map((s) => (
        <group key={s} position={[s * 7.2, 0, -0.5]}>
          <mesh castShadow position={[0, 5, 0]}>
            ...
            <meshStandardMaterial color="#1b1f3a" roughness={0.5} metalness={0.5} />
          </mesh>
          <mesh position={[0, 5, 0]}>
            <boxGeometry args={[0.25, 9.4, 1.5]} />
            <meshStandardMaterial color="#a259ff" emissive="#a259ff" emissiveIntensity={1.8} toneMapped={false} />
          </mesh>
          <mesh position={[0, 10.5, 0]}>
            <octahedronGeometry args={[0.8, 0]} />
            <meshStandardMaterial color="#35f2ff" emissive="#35f2ff" emissiveIntensity={2.6} toneMapped={false} />
          </mesh>
        </group>
      ))}
      {/* ring */}
      <group position={[0, 6.6, 0]}>
        <mesh ref={ring} castShadow>
          ...
          <meshStandardMaterial color="#1b1f3a" roughness={0.35} metalness={0.7} />
        </mesh>
        <mesh>
          <torusGeometry args={[5.2, 0.28, 8, 64]} />
          <meshStandardMaterial ref={rim} color="#35f2ff" emissive="#35f2ff" emissiveIntensity={2.2} toneMapped={false} />
        </mesh>
        <mesh>
          ...
          {Array.from({ length: 10 }).map((_, i) => {
            ...
            return (
              <mesh key={i} position={[Math.cos(a) * 6.6, Math.sin(a) * 6.6, 0]} rotation={[a, a * 0.5, 0]}>
                ...
                <meshStandardMaterial color={i % 2 ? "#35f2ff" : "#ff4fd8"} emissive={i % 2 ? "#35f2ff" : "#ff4fd8"} emissiveIntensity={2.5} toneMapped={false} />
              </mesh>
            );
          })}
        </group>
        <Sparkles count={160} scale={[12, 12, 4]} size={7} speed={1.4} color="#9ff3ff" opacity={0.9} noise={2} />
        <pointLight color="#4fd8ff" intensity={90} distance={45} decay={2} />
      </group>
      <Label text="WEB GATE" sub="CHROME • PRESS E TO ENTER" position={[0, 14.2, 0]} color="#35f2ff" scale={3.2} />
      {targeted && (
